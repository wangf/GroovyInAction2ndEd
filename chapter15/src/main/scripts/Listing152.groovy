...
def url = 'http://www.ibm.com/developerworks/views/java/rss/libraryview.jsp?feed_by=atom'
def atom = new Namespace('http://www.w3.org/2005/Atom')
def numEntries = 3 // show this many entries
def entries = new XmlParser().parse(url)[atom.entry][0..<numEntries]
def len = "dd mmm yyyy ".size() // strip time off published date
